...
class BoundaryAwareTransformer(nn.Module):
    def __init__(self,
        ...
        encoder_layer = BoundaryAwareTransformerEncoderLayer(d_model, nhead,
                                                dim_feedforward, dropout,
                                                activation, normalize_before)
        encoder_norm = nn.LayerNorm(d_model) if normalize_before else None
        self.encoder = BoundaryAwareTransformerEncoder(encoder_layer, num_encoder_layers,
                                          encoder_norm)
        decoder_layer = TransformerDecoderLayer(d_model, nhead,
                                                dim_feedforward, dropout,
                                                activation, normalize_before)
    ...
    def forward(self, src, mask, query_embed, pos_embed):
        ...
        tgt = torch.zeros_like(query_embed)
        memory, weights = self.encoder(src, src_key_padding_mask=mask, pos=pos_embed, height=h, width=w)
        #for i in range(len(weights)):
        #    weights[i] = weights[i].permute(1, 2, 0).contiguous().view(bs, 1, h, w)
...
class BoundaryAwareTransformerEncoderLayer(TransformerEncoderLayer):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1, activation=nn.LeakyReLU, normalize_before=False):
    ...
    def forward(self, src, src_mask: Optional[Tensor] = None, src_key_padding_mask: Optional[Tensor] = None, pos: Optional[Tensor] = None, height: int = 32, width: int = 32):
        if self.normalize_before:
            features = self.forward_pre(src, src_mask, src_key_padding_mask, pos)
            features = features.permute(1,2,0).view(features.shape[1],features.shape[2],height,width)
            weights = torch.sigmoid(self.BAG(features))
            return (features * weights + features).flatten(2).permute(2,0,1).contiguous(), weights
