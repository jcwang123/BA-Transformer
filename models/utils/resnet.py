    ...
    def __init__(self, num_layers):
        ...
        if num_layers == 50:
            ...
            resnet.load_state_dict(
                torch.load(
                    "/home/wjc/.cache/torch/checkpoints/resnet50-19c8e357.pth")
            )
            ...
            resnet.load_state_dict(
                torch.load(
                    "~/.cache/torch/checkpoints/resnet101-5d3b4d8f.pth"))
            # remove fully connected layer, avg pool and layer5:
            self.resnet = nn.Sequential(*list(resnet.children())[:-3])
            ...
            resnet.load_state_dict(
                torch.load(
                    "~/.cache/torch/checkpoints/resnet152-b121ed2d.pth"))
            # remove fully connected layer, avg pool and layer5:
            self.resnet = nn.Sequential(*list(resnet.children())[:-3])
...
class ResNet_BasicBlock_OS16(nn.Module):
    def __init__(self, num_layers):
        ...
        if num_layers == 18:
            ...
            #             load pretrained model:
            resnet.load_state_dict(
                torch.load("~/.cache/torch/checkpoints/resnet18-5c106cde.pth"))
            # remove fully connected layer, avg pool and layer5:
            self.resnet = nn.Sequential(*list(resnet.children())[:-3])
            ...
            # load pretrained model:
            resnet.load_state_dict(
                torch.load("~/.cache/torch/checkpoints/resnet34-333f7ec4.pth"))
            # remove fully connected layer, avg pool and layer5:
            self.resnet = nn.Sequential(*list(resnet.children())[:-3])
